...
void OpenGLES::BindTexture(HANDLE cameraSurface, int width, int height) {
    if (cameraSurface) {
        EGLint attributes[] = {
        ...
        EGLSurface created = eglCreatePbufferFromClientBuffer(mEglDisplay, EGL_D3D_TEXTURE_2D_SHARE_HANDLE_ANGLE, cameraSurface, mEglConfig, attributes);
        if (created == EGL_NO_SURFACE) {
            EGLint error = eglGetError();
            throw Exception::CreateException(E_FAIL, ref new Platform::String(L"Cannot create buffer"));
            return;
        }
        eglBindTexImage(mEglDisplay, created, EGL_BACK_BUFFER);
    }
}
